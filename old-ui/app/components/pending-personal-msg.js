    ...
    return (
      <div key={msgData.id} style={{height: '100%'}}>
        ...
        <div className="flex-row flex-space-around" style={{
          ...
          <button style={{width: '109px'}} onClick={state.cancelPersonalMessage}>Cancel</button>
          <button style={{width: '109px', marginTop: '-40px', marginLeft: '209'}}  onClick={state.signPersonalMessage}>Sign</button>
        </div>
      </div>
    )
  }
